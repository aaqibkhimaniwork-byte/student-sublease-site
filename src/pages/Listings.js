...
export default function Listings() {
  ...
  const toggleFavorite = async (e, listingId) => {
    ...
    if (isFavorited) {
      ...
      if (!error)
        setUserFavorites(userFavorites.filter((id) => id !== listingId));
    } else {
      const { error } = await supabase
        .from("favorites")
  ...
  const processedListings = dbListings
    .filter((listing) => {
      const matchUni =
        !filters.university ||
        listing.universities.some(
          (u) => u.toLowerCase() === filters.university.toLowerCase()
        );
      ...
      const matchZip =
        !filters.zip_code || listing.zip_code.includes(filters.zip_code);

      const matchMoveIn =
        !filters.latestMoveIn ||
        new Date(listing.lease_start) <= new Date(filters.latestMoveIn);

      const matchLeaseEnd =
        !filters.earliestLeaseEnd ||
        new Date(listing.lease_end) >= new Date(filters.earliestLeaseEnd);
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <h3 style={{ margin: 0, color: "#0984e3" }}>
                            ${listing.rent}/mo
                          </h3>
                        </div>

                        <p style={{ color: "#666", margin: "10px 0" }}>
                          {listing.city}, {listing.state} • {listing.sq_ft} sqft
                        </p>
                        ...
                        {isExpanded && (
                          <div style={detailsAreaStyle}>
                            <div style={amenityGridStyle}>
                              ...
                              <div style={amenityItemStyle}>
                                {listing.parking_available ? "✅" : "❌"} Parking
                              </div>
                              <div style={amenityItemStyle}>
                            ...
                            <p>
                              <strong>Available Universities:</strong>{" "}
                              {listing.universities.join(", ")}
                            </p>

                            <p>
                              <strong>Lease Period:</strong> {listing.lease_start}{" "}
                              to {listing.lease_end}
                            </p>
...
const detailsAreaStyle = {
  marginTop: "20px",
  borderTop: "1px solid #eee",
  paddingTop: "20px",
};

const amenityGridStyle = {
  display: "flex",
  gap: "15px",
  marginBottom: "20px",
  flexWrap: "wrap",
